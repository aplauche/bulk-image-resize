...
export default async function processImages(options) {
  ...
  try {
    ...
    if(mode === "none" && outputFormat === "source"){
      console.log("no transformations to perform...")
      return
    }
    ...
    for (const file of files) {
      ...
      if (file.match(/\.(jpg|jpeg|png|gif|webp|tiff)$/i)) {
        ...
        console.log(`Saved ${outputDirectory}/${outputFormat !== "source" ? `${filenameWithoutExtension}.${outputFormat}` : file }`)
      }
    }
