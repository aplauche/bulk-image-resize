...
export default async function processImages(options) {
  ...
  try {
    ...
    if(!!quality){
      fileNameOptionsSuffix += `-q${quality}`
    }
    ...
    for (const file of files) {
      ...
      if (isImage(file)) {
        ...
        if(outputFormat === 'png'){
          await sharp(inputPath).resize(resizeOptions).png({ quality: quality }).toFile(`${outputDirectory}/${finalFileName}.png`); 
        }
        if(outputFormat === 'jpg'){
          await sharp(inputPath).resize(resizeOptions).jpeg({ quality: quality }).toFile(`${outputDirectory}/${finalFileName}.jpg`); 
        }
        if(outputFormat === 'webp'){
          await sharp(inputPath).resize(resizeOptions).webp({ quality: quality }).toFile(`${outputDirectory}/${finalFileName}.webp`); 
        }
